...
#### Change what the algorithms prioritize

(description of intervention).

It's clear that, without outside intervention, this sort of approach is unlikely, as the current algorithm is profit-maximizing, and more engagement leads to more revenue. (something something something, external intervention)

#### Make algorithmic influence much more transparent

(something something)

For example, Facebook could provide an option where a user can learn about why the algorithm provided a certain post to a user's feed.
