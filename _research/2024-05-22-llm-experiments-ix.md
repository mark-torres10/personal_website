...
## Run the pipeline on a cron job on the compute cluster.
Now I can create a cron job for this, based off the cron job for the [sync pipeline](https://markptorres.com/research/llm-experiments-pt-viii). I'll have it run once per day, 6 hours after the sync pipelines.
```bash
#!/bin/bash
